...
function applySettings() {
  ...
  /* صفحة المنتج: لا يُخفى زر إنستغرام بسبب تعارض سمة hidden */
  document.querySelectorAll('a.prod-ig-order-btn').forEach(function (el) {
    if (!igUrl) return;
    el.href = igUrl;
    el.hidden = false;
    try {
...
function buildProductCardHtml(p, opts) {
  ...
  <div class="pc-footer pc-footer--no-social">
...
function renderSkeletons(container, count = 6) {
  ...
  replaceChildrenFromHtml(container, Array(count).fill(0).map(() => `
    <div class="pc pc--product pc--skel" aria-hidden="true">
      ...
      <div class="pc-footer pc-footer--skel pc-footer--no-social">
